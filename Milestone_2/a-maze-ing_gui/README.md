...
The maze generation itself is implemented as a reusable `MazeGenerator` class in the standalone `src/mazegen.py` module. The module can be imported independently and packaged for later reuse, as required by the subject.
...
The project therefore separates the maze generator into the standalone `src/mazegen.py` module.
...
* Support for both perfect and imperfect mazes.
* Integration of the `42` pattern into the maze grid.
* BFS-based shortest-path calculation.
* Maze output generation.
* Integration of the different project components.
* Review, improvement, and integration of the `config_reader`.
* Debugging, testing, and refinement of the overall project.
* Project packaging and build configuration using `pyproject.toml` and `Makefile`.
* Code quality and type checking using `flake8` and `mypy`.

### sshadrin

Responsible primarily for the graphical user interface and configuration component:

* Design and implementation of the interactive GUI using **MLX** (`src/gui.py`).
* Real-time rendering of generated mazes, paths, and visual elements.
* Implementation of event handling and user controls within the MLX environment.
* Initial implementation and structural design of the `config_reader`.
* Parsing and validation of configuration parameters.
* Integration of configuration values into the maze generator and GUI.
* Testing and debugging of the configuration handling.
* Integration of the GUI and configuration components with the rest of the project.
